...
const longitude = Number(geo[0].lon);
  ...
  const cloud_cover = weather.hourly.cloudcover[0] / 100; // convert 0–100 → 0–1
  ...
  const aqi =
    aqiJson.results?.[0]?.measurements?.[0]?.value ?? 50; // fallback if missing

  // 4. Compute shade factor (simple model)
